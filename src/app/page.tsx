...
export default function Home() {
  const [activeWeek, setActiveWeek] = useState<keyof typeof curriculum>("week3");
    ...
    <main id="top" className="relative overflow-x-hidden bg-[#0a0a0a] text-white">
      ...
      <header className="relative border-b border-white/10">
        <div className="mx-auto flex max-w-[1240px] items-center justify-between px-6 py-4 md:px-10 md:py-5">
          <a href="#top" className="text-[1.45rem] font-extrabold tracking-[-0.055em] text-white">FreelancifyAI</a>
          <nav className="hidden items-center gap-12 text-base text-white/55 md:flex">
          ...
          <a href="#pricing" className="inline-flex items-center justify-center rounded-full bg-white px-6 py-3 text-sm font-semibold text-black shadow-[0_18px_40px_rgba(255,255,255,0.12)] transition-transform duration-200 hover:-translate-y-0.5 hover:bg-white/95 md:px-8 md:text-base">Enroll Now — PKR 5,000</a>
        </div>
      </header>

      <section className="px-6 pb-10 pt-24 md:px-10 md:pb-14 md:pt-28">
        <div className="mx-auto flex max-w-[1120px] flex-col items-center text-center">
          <div className="mb-7 inline-flex rounded-full border border-white/10 bg-white/[0.04] px-5 py-2 text-sm font-medium text-white/64 backdrop-blur-sm">3-Week Live Program</div>
          <h1 className="max-w-[12ch] text-balance text-[clamp(3.2rem,8.8vw,7.25rem)] font-extrabold leading-[0.93] tracking-[-0.07em] text-white">Learn to Earn with AI — <span className="text-white/65">Dollar Clients, Rupee Effort</span></h1>
          <p className="mt-8 max-w-4xl text-balance text-lg leading-8 text-white/62 md:text-[1.44rem] md:leading-9">9 live classes. Real tools. Real proposals. Your first client in 3 weeks or you have a system to get there.</p>
          ...
          <p className="mt-8 text-sm font-medium text-white/55 md:text-base">Live on WhatsApp + App · 18 Hours of Instruction · Limited Seats</p>
        </div>
      </section>
      ...
      <section id="program" className="px-6 py-18 md:px-10 md:py-24">
        <div className="mx-auto max-w-[1240px]">
          <SectionHeading title="What You'll Have After 3 Weeks" />
          <div className="mt-14 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {outcomes.map((item) => (
              <article key={item.title} className="rounded-[1.75rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.055),rgba(255,255,255,0.03))] p-6 transition-transform duration-300 hover:-translate-y-1 hover:border-white/16">
                <FeatureMark />
                <h3 className="mt-5 text-xl font-semibold tracking-[-0.035em] text-white">{item.title}</h3>
      ...
      <section className="px-6 py-18 md:px-10 md:py-24">
        <div className="mx-auto max-w-[1240px]">
          <div className="max-w-3xl">
            <SectionHeading align="left" title="Pick Your Path on Day 1. Master It in 3 Weeks." description="You choose one on Class 1. No switching. Full focus." />
          </div>
          <div className="mt-14 grid gap-5 xl:grid-cols-3">
            {paths.map((item) => (
              <article key={item.title} className="rounded-[1.75rem] border border-white/10 bg-white/[0.035] p-7 shadow-[0_0_0_1px_rgba(255,255,255,0.02)_inset]">
                ...
                <div className="mt-3 text-sm font-medium text-white/45">{item.level}</div>
                <p className="mt-4 text-base leading-7 text-white/62">{item.description}</p>
                <div className="mt-6">
      ...
      <section id="curriculum" className="px-6 py-18 md:px-10 md:py-24">
        <div className="mx-auto max-w-[1240px]">
          <SectionHeading title="9 Classes. 18 Hours. Everything You Need." />
          <div className="mt-14 rounded-[2rem] border border-white/10 bg-white/[0.03] p-4 md:p-6">
            ...
            <div className="grid gap-4 p-2 pt-6 md:grid-cols-3 md:p-4">
              {curriculum[activeWeek].classes.map((item) => (
                <article key={item.number} className="rounded-[1.5rem] border border-white/10 bg-[#0d0d0d] p-6">
                  <div className="text-sm font-semibold uppercase tracking-[0.2em] text-white/35">{item.number}</div>
                  <h3 className="mt-4 text-xl font-semibold tracking-[-0.03em] text-white">{item.title}</h3>
                  <p className="mt-3 text-base font-medium text-white/72">{item.subtitle}</p>
                  <p className="mt-3 text-sm leading-7 text-white/48">{item.detail}</p>
                </article>
              ))}
            </div>
        ...
        <div className="mx-auto max-w-[1240px]">
          ...
          <div className="mt-14 grid gap-5 lg:grid-cols-3">
            {[["1", "Week 1 End", "Live Upwork profile · 15 proposals sent · Niche locked"], ["2", "Week 2 End", "3 portfolio projects built · AI delivery workflows live · 45+ proposals sent"], ["3", "Week 3 End", "Sales framework ready · Retainer pitch drafted · 70+ proposals sent"]].map(([step, title, detail]) => (
              <div key={step} className="rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-7">
                <div className="flex items-start gap-5">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-base leading-7 text-white/55">{detail}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="px-6 py-18 md:px-10 md:py-24">
        <div className="mx-auto max-w-[1240px]">
          ...
          <div className="mt-14 grid gap-5 lg:grid-cols-2">
            {[0, 1].map((column) => (
              <div key={column} className="rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-5 md:p-7">
                <div className="grid gap-3">
                  {inclusions.slice(column * 5, column * 5 + 5).map((item) => (
                    <div key={item} className="flex items-center gap-4 rounded-2xl border border-white/8 bg-[#0e0e0e] px-4 py-4">
      ...
      <section id="pricing" className="px-6 py-18 md:px-10 md:py-24">
        <div className="mx-auto max-w-[1240px]">
          <SectionHeading title="One Client Pays for This 10x Over" />
          <div className="mt-14 grid gap-6 xl:grid-cols-2">
            <article className="rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.03))] p-7 md:p-8">
              ...
              <a href="#pricing" className="mt-8 inline-flex h-14 items-center justify-center rounded-full bg-white px-6 text-sm font-semibold text-black transition-transform hover:-translate-y-0.5 hover:bg-white/95 md:px-8 md:text-base">Claim Early Bird Spot</a>
            </article>
            <article className="rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.025))] p-7 md:p-8">
              ...
              <a href="#pricing" className="mt-8 inline-flex h-14 items-center justify-center rounded-full border border-white/15 bg-white/[0.03] px-6 text-sm font-semibold text-white transition-transform hover:-translate-y-0.5 hover:bg-white/[0.06] md:px-8 md:text-base">Enroll at Full Price</a>
            </article>
          </div>
          <div className="mt-8 grid gap-2 text-center text-sm text-white/48 md:text-base">
          ...
          <div className="mt-14 grid gap-4">
            {faq.map((item) => (
              <details key={item.question} className="group rounded-[1.5rem] border border-white/10 bg-white/[0.03] px-6 py-5" open={item.question === "What's the schedule?"}>
                ...
                <p className="mt-4 max-w-3xl text-base leading-7 text-white/58">{item.answer}</p>
              </details>
            ))}
          </div>
        </div>
      ...
      <section className="px-6 py-18 md:px-10 md:py-24">
        <div className="mx-auto max-w-[1240px]">
          <blockquote className="max-w-5xl text-3xl font-semibold tracking-[-0.045em] text-white md:text-5xl md:leading-tight">“The ones who succeed won't be the smartest in the room. They'll be the ones who sent proposals every single day for 90 days straight.”</blockquote>
          <div className="mt-5 text-base font-medium text-white/45">— @thetruehammad</div>
          <p className="mt-4 text-lg text-white/68">Start that 90 days here.</p>
          <a href="#pricing" className="mt-8 inline-flex h-14 items-center justify-center rounded-full bg-white px-8 text-sm font-semibold text-black transition-transform hover:-translate-y-0.5 hover:bg-white/95 md:text-base">Join FreelancifyAI — PKR 5,000</a>
        </div>
      </section>
      ...
      <footer className="border-t border-white/8 px-6 py-10 md:px-10">
        <div className="mx-auto flex max-w-[1240px] flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="text-xl font-extrabold tracking-[-0.05em] text-white">FreelancifyAI</div>
            <div className="mt-1 text-sm text-white/45">Earn with AI. Bill in Dollars.</div>
          </div>
          <a href="#" className="text-sm font-medium text-white/55 transition hover:text-white">@thetruehammad</a>
        </div>
        <div className="mx-auto mt-6 max-w-[1240px] text-sm text-white/35">© 2025 FreelancifyAI. All rights reserved.</div>
      </footer>

      <a href="https://lovable.dev/projects/258e449d-75a2-4219-8926-bd15dfb49d77?utm_source=lovable-badge" target="_blank" rel="noreferrer" className="fixed bottom-4 right-4 z-50 inline-flex items-center gap-2 rounded-xl border border-white/10 bg-[#171717] px-4 py-2 text-sm font-medium text-white/86 shadow-[0_12px_30px_rgba(0,0,0,0.35)]">
        <span>Edit with</span>
